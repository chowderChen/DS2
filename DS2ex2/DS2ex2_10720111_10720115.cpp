...
int main() {
	cout << "(1) 2-3 tree" << endl 
	     << "(2) AVL tree" << endl
	     << "(3) Top K graduates departmant" << endl
	     << "(0) Exit" << endl
	     << "Mission: " ;
	int cmd = 0 ;
	cin >> cmd ;
	...
	while ( cmd != 0 ) {
		if ( cmd != 1 && cmd != 2 && cmd != 3 ) {
			...
			// Top K
			if ( avlTree.IsEmpty() ) {
				cout << "Build AVL tree first." << endl ;
			}
			else {
				cout << "Enter K in [1," << listA.collegeSet.size() << "] : " ;
				int k ;
				cin >> k ;
				if ( k < 1 || k > listA.collegeSet.size() ) {
					cout << "Out of range." << endl ;
				}
				else {
					int printed = 0 ; 
					avlTree.TopKGraduates( avlTree.GetRoot(), k, printed ) ;
				}
			} // else 
		} // else if cmd == 3
		else {
